...
<template>
  <div>
    <el-form class="demo-form-inline query_form" :inline="true">
        <el-form-item>
            <el-input v-model="groupId" placeholder="请输入groupId" style="width: 280px;" clearable></el-input>
        </el-form-item>
        <el-form-item>
          <el-input v-model="projectName" placeholder="请输入projectName" style="width: 280px;" clearable></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table
        ...
        <el-table-column
            label="操作" class-name="not-pre-cell" width="280px">
    ...
    <a ref="exportLink" style="display: none;"></a>
  </div>
</template>
...
<script>
  ...
  export default {
      ...
      methods: {
          // 下载按钮
          handleDownload (row) {
              this.$axios.post(consts.API_CONTEXT + '/api/v1/feign/download/' + row.id, {}, {
                  responseType: 'blob'
              }).then(response => {
          ...
          handleCurrentChange (page) {
              ...
              this.$get(consts.API_CONTEXT + '/api/v1/feign/search', {
                  page: page,
                  size: this.pageSize,
          ...
          handleFeignClient (row) {
              this.$router.push({ path: '/feign/feignClient/clients', query: { projectId: row.id } });
          },
          handleFeignModel (row) {
              this.$router.push({ path: '/feign/feignClient/models', query: { projectId: row.id } });
          }
      }
  }
</script>